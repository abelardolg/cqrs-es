...
use App\Application\Command\User\ChangeEmail\ChangeEmail;
...
class ChangeEmailHandlerTest extends ApplicationTestCase
{
    ...
    {
        $command = new SignUp($uuid = Uuid::uuid4()->toString(), 'asd@asd.asd', 'password');

        $this->handle($command);

        $email = 'lol@asd.asd';

        $command = new ChangeEmail($uuid, $email);
        ...
        /** @var ProjectEmailChanged $emailChangedEmail */
        $emailChangedEmail = $events[1]->getPayload();

        self::assertInstanceOf(ProjectEmailChanged::class, $emailChangedEmail);
